...
function clearContainers() {
  ...
  docker rm -f $(docker ps -aq --filter label=service=fiperledger-fabric) 2>/dev/null || true
...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Creating hosp1apollo Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-hosp1apollo.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
    ...
    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-hosp2vijaya.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
  ...
  while :
    do
      if [ ! -f "organizations/fabric-ca/hosp1apollo/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/hosp1apollo/msp organizations/fabric-ca/hosp1apollo/tls-cert.pem organizations/fabric-ca/hosp1apollo/ca-cert.pem organizations/fabric-ca/hosp1apollo/IssuerPublicKey organizations/fabric-ca/hosp1apollo/IssuerRevocationPublicKey organizations/fabric-ca/hosp1apollo/fabric-ca-server.db'
    docker run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/hosp2vijaya/msp organizations/fabric-ca/hosp2vijaya/tls-cert.pem organizations/fabric-ca/hosp2vijaya/ca-cert.pem organizations/fabric-ca/hosp2vijaya/IssuerPublicKey organizations/fabric-ca/hosp2vijaya/IssuerRevocationPublicKey organizations/fabric-ca/hosp2vijaya/fabric-ca-server.db'
    ...
    docker run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf addInsurance/fabric-ca/Ins1starhealth/msp addInsurance/fabric-ca/Ins1starhealth/tls-cert.pem addInsurance/fabric-ca/Ins1starhealth/ca-cert.pem addInsurance/fabric-ca/Ins1starhealth/IssuerPublicKey addInsurance/fabric-ca/Ins1starhealth/IssuerRevocationPublicKey addInsurance/fabric-ca/Ins1starhealth/fabric-ca-server.db'
...
# use this as the docker compose couch file for hosp3stanley
COMPOSE_FILE_COUCH_ORG3=addOrg3/docker/docker-compose-couch-hosp3stanley.yaml
# use this as the default docker-compose yaml definition for hosp3stanley
COMPOSE_FILE_ORG3=addOrg3/docker/docker-compose-hosp3stanley.yaml
#
