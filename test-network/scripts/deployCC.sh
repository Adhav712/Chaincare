...
CC_SEQUENCE=${6:-"1"}
CC_INIT_FCN=${7:-"NA"}
CC_END_POLICY=${8:-"OR(AND('hosp1apolloMSP.peer', 'hosp2apolloMSP.peer'), AND('hosp1apolloMSP.peer', 'hosp3stanleyMSP.peer'), AND('hosp2apolloMSP.peer', 'hosp3stanleyMSP.peer'))"}
CC_COLL_CONFIG=${9:-"NA"}
DELAY=${10:-"3"}
MAX_RETRY=${11:-"5"}
VERBOSE=${12:-"false"}
